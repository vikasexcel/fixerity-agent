...
async function loadSellerProfileNode(state) {
  try {
    const profile = await SellerProfile.findOne({
      where: { seller_id: state.sellerId, active: true }
    });
    ...
    return { sellerProfile: profile.toJSON() };
  } catch (error) {
...
async function findMatchingJobsNode(state) {
  ...
  try {
    ...
    const whereClause = {
      status: 'open',
      service_category_id: {
        [Op.in]: profile.service_categories
      }
    };

    // Optional: Filter by budget if seller has pricing
    if (profile.pricing?.hourly_rate_min) {
      whereClause['budget.max'] = {
        [Op.gte]: profile.pricing.hourly_rate_min
      };
    }
    ...
    return { 
      matchedJobs: jobs.map(j => j.toJSON()) 
    };
  } catch (error) {
