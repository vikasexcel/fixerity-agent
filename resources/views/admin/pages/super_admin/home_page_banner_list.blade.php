                    ...
                    <div class="row">
                        <div class="col-lg-8">
                            <div class="page-header-title ">
                                ...
                                <div class="d-inline">
                                    <h5> Home Page Banner Image List</h5>
                                    <span>All Banner Image List</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="page-body">
                        <div class="card">
                            <div class="card-header">
                                ...
                                <a href="{{route('get:admin:home_page_banner')}}" title="Add Banner Slider"
                                        ...
                                        @if(isset($banner_image))
                                            @foreach($banner_image as $key => $banner)
                                                ...
                                                <span class="toggle">
                                                    ...
                                                    <td class="action">
                                                        <a class="render_link"
                                                           href="{{ route('get:admin:edit_home_page_banner',[$banner->id]) }}">
                                                            <img
...
@section('page-js')
    ...
    <script type="text/javascript">
        ...
        $(document).on('click', '.delete', function (e) {
            ...
            swal({
                    title: "Are you sure?",
                    text: "You will not be able to recover this app banner!",
                    type: "warning",
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        $.ajax({
                            type: 'get',
                            url: '{{ route('get:admin:delete_home_banner') }}',
                            data: {id: id},
                            success: function (result) {
                                if (result.success == 0) {
                                    swal("Warning", "You can not delete this Service Banner", "success");
                                }
                                if (result.success == true) {
                                    RemovetableRow.remove().draw();
                                    swal("Success", "Service banner Delete Successfully", "success");
                                }
                                else {
        ...
        $(document).on('click', '.home_slider', function (e) {
            ...
            if (status == 1) {
                ...
                txt = "if press yes then disable app banner!";
            } else {
                title = "Enable Home Slider?";
                txt = "if press yes then enable app banner!";
            }
            swal({
                    title: title,
                    text: txt,
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        $.ajax({
                            type: 'get',
                            url: '{{ route("get:admin:change_home_banner_status") }}',
                            data: {id: id},
                            success: function (result) {
                                if (result.success == true) {
