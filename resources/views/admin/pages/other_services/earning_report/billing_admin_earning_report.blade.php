                    ...
                    <div class="page-body">
                        <div class="card">
                            ...
                            <div class="card-block">
                                <form method="post"
                                      action="{{ (Illuminate\Support\Facades\Auth::guard("admin")->check()) ? (Illuminate\Support\Facades\Auth::guard("admin")->user()->roles == 1)? route('post:admin:other_service_earning_report',$slug) : route('post:account:other_service_earning_report') : '' }}">
                                    {{ csrf_field() }}
                                    <div class="row">
                                        ...
                                        <div class="col-lg-4">
                                            <div class="form-group">
                                                <select id="js-example1" name="service_id"
                                                    ...
                                                    @if(isset($service_category_list))
                                                        @if(!$service_category_list->isEmpty())
                                                            @foreach($service_category_list as $key => $service_category)
                                                                {{ $selected = isset($service_id)? ($service_id != Null)? ($service_id == $service_category->service_cat_id)?  "selected" : "" : "" : "" }}
                                                                <option value="{{ $service_category->service_cat_id }}" {{ $selected }}>{{ ucwords($service_category->service_cat_name)  }}</option>
                                                            @endforeach
                                                        @endif
                                                    @endif
                                                </select>
                                        ...
                                        <div class="col-lg-3">
                                            <div class="form-group">
                                                {{--<label></label>--}}
                                                <select id="js-example3" name="provider"
                                                        class="js-example-placeholder-single1 js-states form-control">
                                                    <option disabled selected value=""></option>
                                        ...
                                        <div class="col-lg-12 text-center">
                                            <div class="form-group">
                                                ...
                                                <input type="submit" class="btn btn-default"
                                                       value="Search">
                                                <a href="{{ (Illuminate\Support\Facades\Auth::guard("admin")->check()) ? (Illuminate\Support\Facades\Auth::guard("admin")->user()->roles == 1)? route('get:admin:other_service_earning_report',$slug) : route('post:account:other_service_earning_report') : ''}}"
                                                   class="render_link">
                                ...
                                <form method="post"
                                      action="{{ (Illuminate\Support\Facades\Auth::guard("admin")->check()) ? (Illuminate\Support\Facades\Auth::guard("admin")->user()->roles == 1)? route('post:admin:other_service_order_payment_settle',$slug) : route('post:account:other_service_order_payment_settle') : '' }}">
                                    {{ csrf_field() }}
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="dt-responsive table-responsive" style="{{(isset($package_order_list) && !$package_order_list->isEmpty())? '' : 'display:none' }}">
                                                <table id="new-cons" style="max-width:100% !important;" class="table table-bordered">
                                                    ...
                                                    <tr>
                                                        ...
                                                        <th class="extra-2">Service Type</th>
                                                        <th class="extra-3">Order Id</th>
                                                        <th>Order Date</th>
                                                        <th>Service Time</th>
{{--                                                        <th class="extra">A = Total Order Amount</th>--}}
{{--                                                        <th>B = Site Commission</th>--}}
{{--                                                        <th class="extra">C = Discount</th>--}}
{{--                                                        <th>D = Pay to Provider</th>--}}
{{--                                                        <th class="extra">E = Collect From Provider</th>--}}
{{--                                                        <th>Payment Type</th>--}}
                                                    ...
                                                    @if(isset($package_order_list))
                                                        @foreach($package_order_list as $order)
                                                            ...
                                                            <tr>
                                                                ...
                                                                <th class="extra-2">{{ ($order->service_category_name != Null)? $order->service_category_name : "----" }}</th>
...
{{--                                                                <td><span class="extra "><span class="currency"></span> {{ isset($used_promo_cods[$order->id]) ? $used_promo_cods[$order->id] : 0 }}</span></td>--}}
                                                                <td><span class="extra "><span class="currency"></span> {{ $order->refer_discount }}</span></td>
                                                                <td><span class="extra"><span class="currency"></span> {{ $order->extra_amount > 0 ? round($order->extra_amount,2) : 0 }}</span></td>
                                                    ...
                                                    @if(isset($package_order_list) && !$package_order_list->isEmpty())
                                                        <tr>
                                                            <th colspan="17" style="text-align: right">Total Amount</th>
                                                            <td class=""><span class="currency"></span> {{ isset($total_amount) ? $total_amount : 0 }}</td>
                                                        </tr>
                                                        <tr>
                                                            <th colspan="17" style="text-align: right">Total Site Commission</th>
                                                            <td class=""><span class="currency"></span> {{ isset($site_commission) ? $site_commission : 0 }}</td>
                                                        </tr>
                                                        <tr>
                                                            <th colspan="17" style="text-align: right">Total Provider Earning</th>
                                                            <td class=""><span class="currency"></span> {{ isset($provider_earning) ? $provider_earning : 0 }}</td>
                                                        </tr>
                                                        <tr>
                                                            <th colspan="17" style="text-align: right">Total Promocode
                                                                Discount
                                                            </th>
                                                            <td class=""><span
                                                        ...
                                                        <tr>
                                                            <th colspan="17" style="text-align: right">Total Referral
                                                                Discount
                                                            </th>
                                                            <td class=""><span
                                                                    class="currency"></span> {{ isset($refer_discount) ? $refer_discount : 0 }}
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <th colspan="17" style="text-align: right">Collect From Provider</th>
                                                            <td class=""><span class="currency"></span> {{ isset($collect_from_provider) ? $collect_from_provider : 0 }}</td>
                                                        </tr>
                                                        <tr>
                                                            <th colspan="17" style="text-align:right">Total Provider Outstanding Amount:</th>
                                            ...
                                            <div class="text-center">
                                                @if(isset($package_order_list) && !$package_order_list->isEmpty())
                                                    <input type="submit" class="btn btn-default provider_payment"
                                                           value="Mark As Settled">
                                                @else
...
//            format: "yyyy-mm-dd",
        ...
        $(document).on('click', '.provider_payment', function (e) {
            ...
            var url = '{{route('post:account:other_service_order_payment_settle')}}';
            swal({
