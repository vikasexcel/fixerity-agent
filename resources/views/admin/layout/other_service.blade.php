...
<!-- Style.css -->
    <link rel="stylesheet" type="text/css" href="{{ asset('assets/css/style.css?v=1.2')}}">
            ...
            <div class="pcoded-wrapper">
                ...
                @endif
                    @if(Illuminate\Support\Facades\Auth::guard("on_demand")->check())
                        @if(Illuminate\Support\Facades\Auth::guard('on_demand')->user()->status != 3 && Illuminate\Support\Facades\Auth::guard('on_demand')->user()->verified_at != Null )
                            @include('admin.include.other-service-sidebar')
                        @endif
                    @endif
                {{--sidebar end--}}
