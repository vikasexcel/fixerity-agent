...
;(function ($, window, document, undefined) {
    ...
    var defaults = {
        ...
        cartItemTemplate: '<div class="{veg-nonveg}"><div class="info"><h4 class="list-group-item-heading">{product_name}</h4></div></div>',
        ...
        lang: { // Language variables
            ...
            cartRemove: 'x',
            cartEmpty: '<img src="/assets/images/front/icons/cooking.png"><div>Cart is Empty!<br />Choose your products</div>'
        },
        submitSettings: {
    ...
    $.extend(SmartCart.prototype, {
        ...
        _setToolbar: function () {
            ...
            if (this.options.toolbarSettings.showCheckoutButton) {
                ...
                switch (this.options.toolbarSettings.checkoutButtonStyle) {
                    ...
                    case 'image':
                        ...
                        break;
                    default:
                        btnCheckout = '<button class="btn sc-cart-checkout" type="button">' + this.options.lang.checkout + '</button> ';
                        break;
                }
            ...
            if (this.options.toolbarSettings.showCartSummary) {
                ...
                panelSubTotal.append(this.options.lang.subtotal).append(' <span class="sc-cart-subtotal">0</span><p>Extra Charges may apply</p>');
        ...
        _setEvents: function () {
            ...
            // Item quantity increment event
            $(this.cart_element).on("click", '.cart-minus', function (e) {
                e.preventDefault();

                var fetch_class = "." + this.id;
                var itemId = fetch_class.substring(4, fetch_class.length);
                var itemClass = "." + itemId;
                var numItems = $(".list-group").find(itemClass).val();
                $(itemClass).val(function (i, numItems) {
                    numItems = --numItems;
                    if (numItems == 0) {
                        $(this).parents('.sc-cart-item').fadeOut("normal", function () {
                            mi._removeFromCart($(this).data('unique-key'));
                ...
                mi._updateCartQuantity($(this).parents('.sc-cart-item').data('unique-key'), $(itemClass).val());
            });


            // Item quantity decrement event
            $(this.cart_element).on("click", '.cart-plus', function (e) {
                e.preventDefault();
            ...
            $(this.cart_element).on("change", '.sc-cart-item-qty', function (e) {
                ...
                //Inpul value 0 then remove from cart
                if ($(this).val() == 0) {
                    $(this).parents('.sc-cart-item').fadeOut("normal", function () {
                        mi._removeFromCart($(this).data('unique-key'));
                        $(this).remove();
        ...
        */
        _addUpdateCartItem: function (p) {
            ...
            if (elmMain && elmMain.length > 0) {
                ...
                var itemSummary = '<button class="minus cart-minus" id="sub' + p.unique_key + '"></button>';
                itemSummary += '<input type="text" min="0" max="100" class="sc-cart-item-qty ' + p.unique_key + '" value="' + p[this.options.paramSettings.productQuantity] + '" /><button class="minus cart-plus" id="add' + p.unique_key + '"></button>';
        ...
        _hasCartChange: function () {
            ...
            // var temp_cart_count = $('#temp_cart_count').val();
            // if (temp_cart_count == 0) {
            //     $('.sc-cart-count', this.cart_element).text(temp_cart_count);
            // }
            // else{
            //     $('.sc-cart-count', this.cart_element).text(this.cart.length);
            // }
